		...
		if (state == CallState.End){
			...
			*/
			...
			Notification notification = new Notification.Builder(app)
					.setAutoCancel(true)
					.setContentTitle("Serval Phone Call")
					.setContentText(remotePeer.getDisplayName())
					.setContentIntent(PendingIntent.getActivity(app, 0, myIntent, PendingIntent.FLAG_UPDATE_CURRENT))
					.setSmallIcon(android.R.drawable.stat_sys_phone_call)
					.setWhen(System.currentTimeMillis())
					.build();
			//notification.flags = Notification.FLAG_ONGOING_EVENT;
			nm.notify("Call", ServalBatPhoneApplication.NOTIFY_CALL, notification);
		}
	}
